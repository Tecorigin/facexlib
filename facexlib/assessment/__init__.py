...
def init_assessment_model(model_name, half=False, device='sdaa', model_rootpath=None):
    ...
    state_dict = torch.load(hypernet_model_path, map_location=lambda storage, loc: storage, weights_only=False)
    model.hypernet.load_state_dict(state_dict)
    model = model.eval()
